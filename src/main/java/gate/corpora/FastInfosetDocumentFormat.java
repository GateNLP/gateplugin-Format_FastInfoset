...
@CreoleResource(name = "Fast Infoset Document Format", isPrivate = true, autoinstances = {
    @AutoInstance(hidden = true)}, comment = "Format parser for GATE XML stored in the binary Fast Infoset format", helpURL = "http://gate.ac.uk/userguide/sec:creole:fastinfoset")
public class FastInfosetDocumentFormat extends TextualDocumentFormat {
  ...
  public void unpackMarkup(Document doc, RepositioningInfo repInfo,
    ...
    if((doc == null) || (doc.getSourceUrl() == null
        && doc.getContent() == null)) { throw new DocumentFormatException(
    ...
    /*
     * String content = doc.getContent().toString(); if(content.length() > 2048)
  ...
  private void unpackGateFormatMarkup(Document doc,
    ...
    try {
      ...
      throw new DocumentFormatException(
          "I/O exception for " + doc.getSourceUrl().toString(), ioe);
    }
  }
    ...
    try {
      ...
      try {
        ...
        if(docHasContentButNoValidURL) {
          ...
          docReader = new InputStreamReader(doc.getSourceUrl().openStream(),
              docEncoding);
      ...
      ((DocumentImpl)doc)
          .setNextAnnotationId(xmlDocHandler.getCustomObjectsId());
      ...
      throw new DocumentFormatException(
          "I/O exception for " + doc.getSourceUrl(), e);
    } finally {
